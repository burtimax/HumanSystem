...
namespace DiaryWinFormsNetFramework.Plugins.TaskForm
{
    public partial class TaskForm : BaseFormParent
    {
        ...
        {
            get { return DirConstants.GetProjectDirectory(); }
        }
        ...
        private void CreateAndAddProjectItem(Control parent, bool askNeme = true)
        {
            ...
            AddProjectItemToParentPanel(parent, proj, true);
        }
        ...
        private void AddProjectItemToParentPanel(Control parent, ProjectItem proj, bool needLoadProjectData)
        {
            ...
            ChangeSelectedProjectItem(proj, needLoadProjectData);
        ...
        private void ChangeSelectedTaskItem(TaskItem selectedTaskItem, bool needScroll = false)
        {
            ...
            var form = selectedTaskItem.FindForm();
            Point p = new Point();

            if (needScroll)
            {
                //ToDo здесь осуществить скроллинг до выбранного элемента

                //using (Control c = new Control() { Parent = TaskPanel, Height = 50, Location = selectedTaskItem.Location })
                //{
                //    TaskPanel.ScrollControlIntoView(c);
                //}
            }
        ...
        private bool ProcessHotKeysForTaskItems(ref Message msg, Keys keyData)
        {
            if (this.SelectedProjectItem?.SelectedTaskItem == null &&
                this.SelectedProjectItem?.SelectedTaskItem?.IsRootTask() == false) return false;
            ...
            if (keyData == (Keys.Control | Keys.W))
            {
                if (this.SelectedProjectItem?.SelectedTaskItem != null)
                {
                    ...
                    SelectNextTaskItem(taskForDelete);
        ...
        private void SelectNextTaskItem(TaskItem curTaskItem, bool goToChildren = true)
        {
            ...
            {
                ChangeSelectedTaskItem(curTaskItem.SubTaskItems[0], true);
                return;
            }
            ...
            {
                ChangeSelectedTaskItem(curTaskItem.ParentTaskItem.SubTaskItems[nextIndex], true);
                return;
            }
        ...
        private void SelectPreviousTaskItem(TaskItem curTaskItem, bool goToChildren = true)
        {
            ...
            {
                ChangeSelectedTaskItem(curTaskItem.ParentTaskItem, true);
                return;
            }
                ...
                prevTaskItem.Task.Revealed == true)
            {
                ...
                ChangeSelectedTaskItem(GetLastSubtaskFromTaskItem(prevTaskItem), true);
                return;
            }

            ChangeSelectedTaskItem(prevTaskItem, true);
        }
        ...
        private void LoadProjectsList()
        {
            ...
            foreach (var file in projFiles)
            {
                ...
                //Для того, чтобы последний проект загружал контент, а остальным это ни к чему.
                if (file == projFiles.LastOrDefault())
                {
                    needLoadData = true;
                }
                AddProjectItemToParentPanel(this.ProjectsPanel, projItem, needLoadData);
            }
        }
